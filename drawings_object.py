...
def writeImage(image):
    ...
    g = image.copy()
    b = image.copy()
    label_colours = np.array([nodule, Unlabelled])
    for l in range(0,2):
...
def display_color_legend():
    ...
    nodule          = np.array([128,128,128])/256
    Unlabelled   = np.array([0,128,192])/256

    patches = [mpatches.Patch(color=nodule, label='nodule'), mpatches.Patch(color=Unlabelled, label='Unlabelled')]
